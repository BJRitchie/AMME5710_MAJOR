...
# sfm_pipeline.resize_ims( store_path, 1200, 2 )  # originally 10
...
# # ---------------------------------------------------------
# # 1. Example pose data (replace these with your actual data)
# # ---------------------------------------------------------
# # Each pose = (R, t) where R is 3x3 rotation, t is 3x1 translation
# # Assume both arrays are N x (R|t)
# # num_images = 5  # example
# # sfm_rotations = [np.eye(3) for _ in range(num_images)]  # replace with SfM R_i
# # sfm_translations = [np.zeros((3, 1)) for _ in range(num_images)]  # replace with SfM t_i

# sfm_rotations, sfm_translations = sfm_pipeline.get_poses()
# checker_rotations, checker_translations = cb.get_poses()

# # checker_rotations = [np.eye(3) for _ in range(num_images)]  # replace with checkerboard R_i
# # checker_translations = [np.zeros((3, 1)) for _ in range(num_images)]  # replace with checkerboard t_i

# # ---------------------------------------------------------
# # 2. Known transform from checkerboard → satellite (placeholder)
# # ---------------------------------------------------------
# # This expresses the checkerboard’s position and orientation relative to the satellite body

# # TODO REPLACE WITH ACTUAL TRANSFORM FROM CAD 
# R_cb_to_sat = o3d.geometry.get_rotation_matrix_from_xyz([0.1, 0.0, 0.0])  # small rotation
# t_cb_to_sat = np.array([[0.05], [0.02], [0.01]])  # in meters
...
# # ---------------------------------------------------------
# # 3. Convert checkerboard poses to satellite frame
# # ---------------------------------------------------------
# T_sat_to_cam_list = []
...
# rotation_diffs = []
# translation_diffs = []
# num_images = cb.get_num_detected()  # number of images where checkerboard was detected

# for idx_in_cb in range(num_images):
#     i = cb.detected_indices[idx_in_cb]  # get the original image index
